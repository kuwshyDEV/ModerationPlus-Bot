...
class BanCog(commands.Cog):
    """Cog for /ban with robust handling."""
    def __init__(self, b: commands.Bot):
        self.b = b
        self.db = CaseDB()

    @app_commands.command(name="ban", description="Ban user with optional proof")
    ...
    async def ban(self, it: discord.Interaction, user: discord.Member, reason: str=None, image1: discord.Attachment=None, image2: discord.Attachment=None, image3: discord.Attachment=None, image4: discord.Attachment=None):
        """Execute ban, log case."""
        ...
        try:
            await user.ban(reason=reason or "Test")
        ...
        case_id = self.db.add_case(it.user.id, user.id, "Ban", reason or "Test", urls)
        c = CaseContainer(it.user, user, "Ban", case_id, urls, reason or "Test")
        v = ui.LayoutView()
        v.add_item(c)
        await it.followup.send(view=v, files=files)

async def setup(b: commands.Bot):
    await b.add_cog(BanCog(b))